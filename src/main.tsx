...
import { containsWholeTermIgnoreCase } from './whole-word-match.js';
import './styles.css';
import './help-tip.css';
...
function makeFreq(tokens: string[], n: number, ignoreCase: boolean, excludeStop = false, excludeNumber = false): Frequency[] {
  ...
  for (let i = 0; i <= tokens.length - n; i++) {
    const raw = n === 1 ? tokens[i] : tokens.slice(i, i + n).join(' '); const key = ignoreCase ? raw.toLocaleLowerCase() : raw;
...
function App() {
  ...
  const undo = () => { if (historyIndex.current >= 0) { setText(history.current[historyIndex.current]); historyIndex.current--; } };
  ...
  const frequencyData = stats[activeFrequency]; const frequencyQueryKey = frequencyCaseSensitive ? frequencyQuery : frequencyQuery.toLocaleLowerCase(); const filteredFrequencyData = useMemo(() => frequencyQueryKey ? frequencyData.filter(item => (frequencyCaseSensitive ? item.term : item.term.toLocaleLowerCase()).includes(frequencyQueryKey)) : frequencyData, [frequencyCaseSensitive, frequencyData, frequencyQueryKey]); const frequencyPages = Math.max(1, Math.ceil(filteredFrequencyData.length / limit)); const frequencyStart = (frequencyPage - 1) * limit; useEffect(() => setPageDraft(String(frequencyPage)), [frequencyPage]);
  ...
  const upload = async (list: FileList | null) => { if (!list) return; const parsed = await Promise.all(Array.from(list, async (file): Promise<CsvData> => { try { const data = csvParse(await file.arrayBuffer()); return { ...data, name: file.name }; } catch { return { name: file.name, headers: [], rows: [], error: '解析失败' }; } })); setFiles((previous) => [...previous, ...parsed]); const cols = [...new Set(parsed.flatMap(f => f.headers))]; setSelectedColumns((previous) => [...previous, ...cols.filter((col) => !previous.includes(col))]); setStatus(`已读取 ${parsed.length} 个文件，共 ${parsed.reduce((n, f) => n + f.rows.length, 0)} 行`); };
  const buildWorkbook = async (force = false) => { if (isExporting) return; if (!files.length || !column) { setStatus('请先上传 CSV 并选择检索列'); return; } if (!mainTerms.length && !otherTerms.length && !force) { setWarningMessage('主词条和其他词条都为空，继续后将按当前设置生成页签。'); setWarningContinue(() => () => { void buildWorkbook(true); }); return; } const valid = files.filter(f => f.headers.includes(column)); const headers = [...new Set(valid.flatMap(f => f.headers))]; const translationTargets = translationEnabled ? translationColumns.filter(header => headers.includes(header) && selectedColumns.includes(header)) : []; const exportHeaders = headers.filter((header) => selectedColumns.includes(header) || translationTargets.includes(header) || header === sortColumn || (highlightRank && header === '搜索频率排名')); const outputHeaders = exportHeaders.flatMap(header => translationTargets.includes(header) ? [header, `${header}（翻译）`] : [header]); const allRows = valid.flatMap(f => f.rows.map(r => Object.fromEntries(headers.map(h => [h, r[f.headers.indexOf(h)] ?? ''])))); const all = sortColumn ? [...allRows].sort((a, b) => String(a[sortColumn] ?? '').localeCompare(String(b[sortColumn] ?? ''), undefined, { numeric: true, sensitivity: 'base' })) : allRows; const rootNames = new Set(valid.flatMap(f => { const fileName = f.name.trim().toLocaleLowerCase(); return [fileName, fileName.replace(/\.csv$/i, '')]; })); const isRootRow = (row: Record<string, string>) => rootNames.has(String(row[column] ?? '').trim().toLocaleLowerCase()); const rootRows = all.filter(isRootRow); const root = sortColumn ? [...rootRows].sort((a, b) => String(a[sortColumn] ?? '').localeCompare(String(b[sortColumn] ?? ''), undefined, { numeric: true, sensitivity: 'base' })) : rootRows; const filteredAll = rootTab && excludeEarlierMainMatches ? all.filter(row => !isRootRow(row)) : all; const translations = new Map<string, Map<string, string>>(); const termTranslations = new Map<string, string>(); if (translationEnabled) { const values = [...new Set([...all.flatMap(row => translationTargets.map(header => String(row[header] ?? '').trim())), ...mainTerms].filter(Boolean))]; const automaticFallbackLanguage = detectDominantTranslationLanguage(values); const translationItems = [...new Map(values.map(value => { const sourceLanguage = resolveTranslationLanguage(value, translationSourceLanguage, automaticFallbackLanguage); return [translationCacheKey(sourceLanguage, value), { value, sourceLanguage }]; })).values()]; if (translationItems.length) { setIsExporting(true); try { const translatedValues = await translateViaBaidu(translationItems, (completed, total) => setStatus(`正在通过百度大模型翻译 ${completed}/${total} 条数据和页签…`)); values.forEach(value => translationTargets.forEach(header => { if (!translations.has(header)) translations.set(header, new Map()); const sourceLanguage = resolveTranslationLanguage(value, translationSourceLanguage, automaticFallbackLanguage); translations.get(header)!.set(value, translatedValues.get(translationCacheKey(sourceLanguage, value)) ?? value); })); mainTerms.forEach(term => { const sourceLanguage = resolveTranslationLanguage(term, translationSourceLanguage, automaticFallbackLanguage); termTranslations.set(term, translatedValues.get(translationCacheKey(sourceLanguage, term)) ?? term); }); } catch (error) { setStatus(`在线翻译失败：${error instanceof Error ? error.message : '请稍后重试'}`); return; } finally { setIsExporting(false); } } } const wb = XLSX.utils.book_new(); const used = new Set<string>(); const add = (name: string, rows: Record<string, string>[], sheetName = name) => { const outputRows = rows.map(row => { const output: Record<string, string> = {}; exportHeaders.forEach(header => { const value = String(row[header] ?? ''); output[header] = value; if (translationTargets.includes(header)) output[`${header}（翻译）`] = value ? (translations.get(header)?.get(value.trim()) ?? value) : ''; }); return output; }); const ws = XLSX.utils.json_to_sheet(outputRows.length ? outputRows : [Object.fromEntries(outputHeaders.map(h => [h, '']))], { header: outputHeaders }); outputHeaders.forEach((_, columnIndex) => { const cell = ws[XLSX.utils.encode_cell({ r: 0, c: columnIndex })]; if (cell) cell.s = { font: { bold: true, color: { rgb: 'FFFFFFFF' } }, fill: { fgColor: { rgb: 'FF17212B' } }, alignment: { horizontal: 'center', vertical: 'center', wrapText: true }, border: { top: { style: 'thin', color: { rgb: 'FFCBD8E2' } }, bottom: { style: 'thin', color: { rgb: 'FFCBD8E2' } }, left: { style: 'thin', color: { rgb: 'FFCBD8E2' } }, right: { style: 'thin', color: { rgb: 'FFCBD8E2' } } } }; }); ws['!rows'] = [{ hpt: 24 }]; ws['!cols'] = outputHeaders.map(header => { const lengths = [header, ...outputRows.map(row => String(row[header] ?? '') )].map(value => Array.from(value).reduce((width, char) => width + (char.charCodeAt(0) > 255 ? 2 : 1), 0)).sort((a, b) => a - b); const percentileIndex = Math.min(lengths.length - 1, Math.ceil(lengths.length * 0.9) - 1); const headerWidth = Array.from(header).reduce((width, char) => width + (char.charCodeAt(0) > 255 ? 2 : 1), 0) + 2; const dataWidth = Math.min(40, (lengths[percentileIndex] ?? 10) + 2); return { wch: Math.max(10, headerWidth, dataWidth) }; }); if (highlightRank && exportHeaders.includes('搜索频率排名')) { rows.forEach((row, rowIndex) => { const rank = Number(String(row['搜索频率排名'] ?? '').replace(/[,，\s]/g, '')); const color = rank <= 30000 && rank > 0 ? 'FFFDE047' : rank > 30000 && rank <= 150000 ? 'FFFDBA74' : null; if (color) outputHeaders.forEach((_, columnIndex) => { const cell = ws[XLSX.utils.encode_cell({ r: rowIndex + 1, c: columnIndex })]; if (cell) cell.s = { ...(cell.s ?? {}), fill: { patternType: 'solid', fgColor: { rgb: color } }, border: { top: { style: 'thin', color: { rgb: 'FF64748B' } }, bottom: { style: 'thin', color: { rgb: 'FF64748B' } }, left: { style: 'thin', color: { rgb: 'FF64748B' } }, right: { style: 'thin', color: { rgb: 'FF64748B' } } } }; }); }); } XLSX.utils.book_append_sheet(wb, ws, cleanSheetName(sheetName, used)); }; if (rootTab) add('根词', root); const matchesAnyTerm = (row: Record<string, string>, terms: string[]) => terms.some(term => containsWholeTermIgnoreCase(row[column], term)); const unfiltered = filteredAll.filter(row => !matchesAnyTerm(row, mainTerms) && !matchesAnyTerm(row, otherTerms)); add('未筛选', unfiltered); const matchedByEarlierMain = new Set<Record<string, string>>(); mainTerms.forEach(term => { const matches = filteredAll.filter(row => containsWholeTermIgnoreCase(row[column], term) && (!excludeEarlierMainMatches || !matchedByEarlierMain.has(row))); add(term, matches, translationEnabled ? translatedSheetName(term, termTranslations.get(term)) : term); if (excludeEarlierMainMatches) matches.forEach(row => matchedByEarlierMain.add(row)); }); const other = filteredAll.filter(row => (!excludeEarlierMainMatches || !matchesAnyTerm(row, mainTerms)) && matchesAnyTerm(row, otherTerms)); add('其他', other); add('总数据', all); XLSX.writeFile(wb, `词频汇总_${new Date().toISOString().slice(0, 10)}.xlsx`); setDownloadReady(true); setStatus(`筛选完成：${valid.length} 个文件，总数据 ${all.length} 行${translationEnabled ? '，已添加百度大模型翻译' : ''}`); };
...
</div>}{isDraggingFiles && <div className="drag-overlay" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); e.stopPropagation(); setIsDraggingFiles(false); if (e.dataTransfer.files.length) upload(e.dataTransfer.files); }}>
...
<option value="">请选择列名</option>{allHeaders.map(c => <option key={c}>{c}</option>)}</select>
</label>
...
<option value="">不排序</option>{allHeaders.map(c => <option key={c}>{c}</option>)}</select>
</label>
...
<b>导出数据列</b>
<div>
<button onClick={() => setSelectedColumns(allHeaders)}>全选</button>
<button onClick={() => setSelectedColumns([])}>清空</button>
</div>
</div>
<div className="column-options">{allHeaders.map(header => <label key={header}>
...
<label className="translation-language">源语言
<select value={translationSourceLanguage} onChange={e => setTranslationSourceLanguage(e.target.value as TranslationSourceLanguage)}>
<option value="auto">自动识别（混合语言）</option>
<option value="deu_Latn">德语（推荐用于德语词库）</option>
<option value="eng_Latn">英语</option>
<option value="fra_Latn">法语</option>
</select>
</label>
<div className="column-options">{allHeaders.filter(header => selectedColumns.includes(header)).map(header => <label key={header}>
...
<TranslationCost files={files} column={column} translationColumns={translationColumns} selectedColumns={selectedColumns} cache={translationCache} sourceLanguage={translationSourceLanguage} />
...
function TranslationCost({ files, column, translationColumns, selectedColumns, cache, sourceLanguage }: { files: CsvData[]; column: string; translationColumns: string[]; selectedColumns: string[]; cache: Map<string, TranslationCacheEntry>; sourceLanguage: TranslationSourceLanguage }) {
  const { values, automaticFallbackLanguage, ambiguous, cached, chars } = useMemo(() => {
    ...
    const targets = translationColumns.filter(header => headers.has(header) && selected.has(header));
    const uniqueValues = [...new Set(valid.flatMap(file => {
      const columnIndexes = targets.map(header => file.headers.indexOf(header));
      return file.rows.flatMap(row => columnIndexes.map(index => String(row[index] ?? '').trim()));
    }).filter(Boolean))];
    const fallbackLanguage = detectDominantTranslationLanguage(uniqueValues);
    let ambiguousCount = 0;
    let cachedCount = 0;
    let pendingCharacters = 0;
    for (const value of uniqueValues) {
      const detected = detectTranslationLanguage(value);
      if (detected.language !== 'zho_Hans' && !detected.confident) ambiguousCount++;
      const language = resolveTranslationLanguage(value, sourceLanguage, fallbackLanguage);
      if (language === 'zho_Hans') continue;
      if (cache.has(translationCacheKey(language, value))) cachedCount++;
      else pendingCharacters += Array.from(value).length;
    }
    return { values: uniqueValues, automaticFallbackLanguage: fallbackLanguage, ambiguous: ambiguousCount, cached: cachedCount, chars: pendingCharacters };
  }, [cache, column, files, selectedColumns, sourceLanguage, translationColumns]);
  return <p className="translation-note"><span>使用限额为1百万字符/月，尽量只翻译最终版本。</span>{sourceLanguage === 'auto' && values.length > 0 && <span className="translation-note-language">自动识别主语言：{TRANSLATION_LANGUAGE_NAMES[automaticFallbackLanguage]}；{ambiguous.toLocaleString()} 个歧义词按主语言处理，明确的其他语种仍会单独识别。</span>}<span className="translation-note-warning">翻译有可能会生成一个.csv后缀的文件，请把这个文件发给网站管理员！</span><span className="translation-note-meta">缓存命中：{Math.max(0, cached).toLocaleString()} 条；本次预计消耗：{chars.toLocaleString()} 字符</span></p>;
}
...
function FrequencyGroup({title,data,total,offset,denominator,main,other,addTerm}:{title:string;data:Frequency[];total:number;offset:number;limit:number;denominator:number;main:string[];other:string[];addTerm:(t:string,k:"main"|"other")=>void}) {
  const mainTerms = useMemo(() => new Set(main), [main]);
  const otherTerms = useMemo(() => new Set(other), [other]);
...
<button className={"term-action main-action " + (mainTerms.has(x.term) ? "added" : "")} onClick={() => addTerm(x.term,"main")}>{mainTerms.has(x.term)?"✓ 主词条":"主词条"}</button>
<button className={"term-action other-action " + (otherTerms.has(x.term) ? "added" : "")} onClick={() => addTerm(x.term,"other")}>{otherTerms.has(x.term)?"✓ 其他":"其他"}</button>
</div>)}</div>;
}

function TermColumn({title,terms,kind,manual,setManual,addManual,setTerms,move,onClear}:{title:string;terms:string[];kind:'main'|'other';manual:string;setManual:(v:string)=>void;addManual:(k:'main'|'other')=>void;setTerms:(v:string[])=>void;move:(k:'main'|'other',i:number,d:-1|1)=>void;onClear:()=>void}) { return <div className="term-column">
