...
function TranslationCost({ files, column, translationColumns, selectedColumns, cache, sourceLanguage }: { files: CsvData[]; column: string; translationColumns: string[]; selectedColumns: string[]; cache: Map<string, TranslationCacheEntry>; sourceLanguage: TranslationSourceLanguage }) {
  ...
  const automaticFallbackLanguage = detectDominantTranslationLanguage(values);
  const ambiguous = values.filter(value => { const detected = detectTranslationLanguage(value); return detected.language !== 'zho_Hans' && !detected.confident; }).length;
  const pending = values.filter(value => {
    const detectedLanguage = resolveTranslationLanguage(value, sourceLanguage, automaticFallbackLanguage);
    return detectedLanguage !== 'zho_Hans' && !cache.has(translationCacheKey(detectedLanguage, value));
  });
  const cached = values.length - pending.length - values.filter(value => resolveTranslationLanguage(value, sourceLanguage, automaticFallbackLanguage) === 'zho_Hans').length;
