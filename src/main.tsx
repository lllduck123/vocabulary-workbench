...
type TranslationLanguage = 'zho_Hans' | 'eng_Latn' | 'deu_Latn' | 'fra_Latn';
type TranslationSourceLanguage = 'auto' | Exclude<TranslationLanguage, 'zho_Hans'>;

function detectTranslationLanguage(value: string): TranslationLanguage {
  const compact = value.replace(/\s/g, '');
  const chinese = (compact.match(/[\u4e00-\u9fff]/g) ?? []).length;
  if (chinese && chinese >= compact.length * 0.35) return 'zho_Hans';
  const words = value.toLocaleLowerCase().match(/[a-zà-öø-ÿ]+/g) ?? [];
  const score = (terms: string[]) => words.reduce((total, word) => total + (terms.includes(word) ? 1 : 0), 0);
  if (/[äöüß]/i.test(value) || score(['der', 'die', 'das', 'und', 'ist', 'nicht', 'mit', 'für', 'von', 'auf', 'ein', 'eine']) >= 2) return 'deu_Latn';
  if (/[àâçéèêëîïôûùüÿœ]/i.test(value) || score(['le', 'la', 'les', 'de', 'des', 'et', 'est', 'avec', 'pour', 'un', 'une', 'dans']) >= 2) return 'fra_Latn';
  return 'eng_Latn';
}

function resolveTranslationLanguage(value: string, sourceLanguage: TranslationSourceLanguage): TranslationLanguage {
  return sourceLanguage === 'auto' ? detectTranslationLanguage(value) : sourceLanguage;
}
...
<option value="auto">自动识别（长句适用）</option>
...
function TranslationCost({ files, column, translationColumns, selectedColumns, cache, sourceLanguage }: { files: CsvData[]; column: string; translationColumns: string[]; selectedColumns: string[]; cache: Map<string, TranslationCacheEntry>; sourceLanguage: TranslationSourceLanguage }) {
  ...
  const pending = values.filter(value => {
    const detectedLanguage = resolveTranslationLanguage(value, sourceLanguage);
    return detectedLanguage !== 'zho_Hans' && !cache.has(translationCacheKey(detectedLanguage, value));
  });
  const cached = values.length - pending.length - values.filter(value => resolveTranslationLanguage(value, sourceLanguage) === 'zho_Hans').length;
  const chars = pending.reduce((total, value) => total + Array.from(value).length, 0);
  return <p className="translation-note"><span>使用限额为1百万字符/月，尽量只翻译最终版本。</span>{sourceLanguage === 'auto' && <span className="translation-note-warning">短词可能跨语言歧义，例如德语 tee 是“茶”；德语词库请把源语言切换为“德语”。</span>}<span className="translation-note-warning">翻译有可能会生成一个.csv后缀的文件，请把这个文件发给网站管理员！</span><span className="translation-note-meta">缓存命中：{Math.max(0, cached).toLocaleString()} 条；本次预计消耗：{chars.toLocaleString()} 字符</span></p>;
}
